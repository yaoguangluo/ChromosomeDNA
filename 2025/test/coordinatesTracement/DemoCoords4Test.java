 ...
 * */
/*
 * 这是一个 处理商旅簇 isolation Isolation_X_getTSPIsolationGroups2D 测试main函数demo的test版本，在导入了api之后进行系统集成，然后
...
//		//处理商旅簇isolation
        Map<Double, List<AMV_MVS_VSQ_2D>> output = Isolation_X_getTSPIsolationGroups2D._E(input, 12);
    ...
    public static void _E(List<AMV_MVS_VSQ_2D> inputs) {
        //处理商旅簇isolation
        Map<Double, List<AMV_MVS_VSQ_2D>> output = Isolation_X_getTSPIsolationGroups2D._E(inputs, 12);
        Iterator<Double> iterator = output.keySet().iterator();
        while (iterator.hasNext()) {
            double temp = iterator.next();
            System.out.println();
