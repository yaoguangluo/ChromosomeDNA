...
public class DemoCoords1Test {
    ...
    public static void _E_(List<AMV_MVS_VSQ_2D> inputs) {
        inputs = new ArrayList<>();
        addNewPostion(inputs, 0.0, 0.0);
        addNewPostion(inputs, 5.0, 0.0);
        addNewPostion(inputs, 5.0, 5.0);
        addNewPostion(inputs, 0.0, 5.0);
        addNewPostion(inputs, -5.0, 5.0);
        addNewPostion(inputs, -5.0, 0.0);
        addNewPostion(inputs, -4.0, -5.0);
        addNewPostion(inputs, -5.0, -5.0);
        addNewPostion(inputs, -5.0, -6.0);
        addNewPostion(inputs, 0.0, -5.0);
        addNewPostion(inputs, 5.0, -5.0);
        //处理 side end
        List<AMV_MVS_VSQ_2D> output = SideEnd_X_getSideEnd2D._E(inputs, 3);//.getSideEnd2D(input, 10);
