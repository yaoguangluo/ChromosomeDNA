...
//	headers.append('Access-Control-Allow-Origin', '*');
	...
	$scope.logout = function (email, token) {	
		...
		$http.get('logout?uEmail=' + encodeURIComponent(email)
				+'&uToken=' + encodeURIComponent(token))
	...
	$scope.shake = function(){//用来确定闪烁的窗体位置
		document.getElementById('audio').play();
		$("#right-info-bar").delay("fast").animate({width:'310px'});
		$("#right-info-bar").delay("fast").animate({width:'230px'});
		$("#right-info-bar").delay("fast").animate({width:'290px'});
		$("#right-info-bar").animate({width:'270px'});
	}
	...
	$scope.ask = function (askMessage) {
		...
		$http.post('ask?ip=' + encodeURIComponent($cookieStore.get("ip"))
				...
				+ '&pointIp=' + encodeURIComponent($scope.pointIp))
				.then(function successCallback(response) {
