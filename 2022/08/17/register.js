 ...
 * 作者， 著作权人： 罗瑶光， 浏阳
 * */
var am = angular.module("register", ['ngCookies']);
am.controller('register', ['$cookieStore', '$scope', '$http', function ($cookieStore, $scope, $http) {
...
//		headers.append('Access-ip', ip);
	...
	$scope.register = function (usrname, email, password, repassword) {	
		...
		$http.get('register?uEmail=' + encodeURIComponent(email) 
				...
				.then(function successCallback(response) {
					...
					if($scope.loginInfo == 'success'){
						...
						window.location.href = "makeRegister.html";
						alert("登陆失败！" + $scope.loginInfo + ' ' + $scope.returnResult);
					}
				}, function errorCallback(response) {
					$scope.backFeed = "invalid registeration！";
					alert("注册失败！" + $scope.email);
				});
	}
}]);